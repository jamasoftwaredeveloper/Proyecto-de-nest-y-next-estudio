...
export class AuthService {
    ...
    async register({ name, email, password, type }: RegisterDto) {
        ...
        const userNew = await this.usersService.create({ name, email, password: await bcryptjs.hash(password, 10), type });
    ...
    async generateRefreshToken(user): Promise<string> {
        ...
        const payload = { email: user.email, sub: user.id, role: user.role, iss: process.env.URN_SYSTEM_REFRESH };
    ...
    async generateAccessToken(user): Promise<string> {
        ...
        const payload = { email: user.email, sub: user.id, role: user.role, iss: process.env.URN_SYSTEM_ACCESS };
